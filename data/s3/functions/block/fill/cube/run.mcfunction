...
execute if score max_len tmp <= len tmp run say [s3:block/fill/run]:执行数可能超过连锁上限！已终止执行！
# say a

data modify storage temp block_store set from storage temp block.get
# 确定执行点
execute unless score max_len tmp <= len tmp run summon area_effect_cloud ~ ~ ~ {Tags:["tmp","s3.fill"]}
execute unless score max_len tmp <= len tmp as @e[tag=tmp,tag=s3.fill] run function s3:block/fill/cube/as_marker
